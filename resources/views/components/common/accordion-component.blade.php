<div class="flex flex-col select-none py-6 relative overflow-hidden">
    ...
    <div class="flex flex-row justify-between w-full {{ $class }} ">
        @if ($position != -1)
            <p class="basis-1/6 text-2xl ssm:text-xl xs:text-xl font-dm-sans text-partial">
                {{ str_pad($position, 2, '0', STR_PAD_LEFT) }}
            </p>
        @endif
        <p class="basis-5/6 text-2xl ssm:text-xl xs:text-xl font-dm-sans text-partial ">{{ $title }}</p>
    </div>
