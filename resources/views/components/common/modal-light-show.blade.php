<div id="lightShowModal" style="display: none" class="fixed z-10 left-0 top-0 w-screen h-screen overflow-hidden bg-black opacity-50">
    <span class="text-white absolute top-3 right-6 text-4xl hover:cursor-pointer" onclick="closeModal()">&times;</span>
    <div class="relative m-auto p-0 w-11/12 max-w-7xl">
        @foreach ($album->photos as $key => $photo)
            <div style="display: none" class="modal-slide">
                ...
                <img src="https://admin.natadesign.id/{{ $photo->original }}" loading="lazy" width="550" height="550"
                    alt="{{ $photo->alt }}" class="select-none aspect-square w-9/12 absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2">
            </div>
        @endforeach
